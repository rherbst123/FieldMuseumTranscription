...
# returns the value of the get_levenshtein_distance_function() if there is no match 
def is_match(val1, val2):
    return val1.strip().lower() == val2.strip().lower() or get_levenshtein_distance(val1, val2)
            ...
            if not is_match(grtruth_val, "N/A"):
                ...
                # just in case there is not a valid target (i.e., "N/A" in the ground truth)
                   # and the llm still transcribes "N/A", the number of overall targets will still be incremented.
                     # for this reason, the number of targets does depend a little on the llm run
...
def process(run_spreadname, ground_truth_dicts, blank_results_dict):
    ...
    results_dict = compare_and_tally(transcription_dicts, ground_truth_dicts, blank_results_dict)  
    return results_dict
