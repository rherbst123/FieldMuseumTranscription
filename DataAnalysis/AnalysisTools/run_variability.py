...
FILENAMES = [["completed_agreement_gpt_sonnet_gemini.csv", "completed_agreement_gpt_sonnet_gemini2.csv"]]              
# Enter below the filename for the Ground Truth source
GROUND_TRUTH_FILENAME = "First100BryophytesTyped.csv"
#GROUND_TRUTH_FILENAME = "78BryophytesTyped.csv"
...
RESULTS_FILENAME = "gpt_sonnet_gemini_variability.csv"
...
def get_field_variabilty_as_percents(d): 
    for fieldname, sums in d.items():
        if sums[2] == 0:
            d[fieldname][2] = d[fieldname][0] if d[fieldname][0] != 0 else 1
